...
function died(event) {
    ...
    var pool0 = itemService.fetch("3");//fetches an item from the itemizer item pool string of pool ID

    ///*/// if rng greater than or equal to 30 do some math to get the amount of coins given to the player.
        if(rng >= 30){
              var econLoot = getRandomInt(3,10)
    ...
    ///*///
      ...
      //checks if pool0 returns an item, if yes perform item drop.
        if(pool0){
          itemService.instanciate(pool0,event.npc.getWorld().getName(),event.npc.getBlockX(),event.npc.getBlockY(),event.npc.getBlockZ()); //create dropped item at location
        }
}
